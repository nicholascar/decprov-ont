...
Figure 14 adds DecPROV elements to the scenario in Figure 13 indicating that before taking out the rubbish, Agent Nick faced the Question What should Nick do? and within the DecisionMaking triggered by that Question he faced two OptionSelection scenarios: one to Choose to do modelling with an associated Requirement that [he] have fun, another to Choose to take out the rubbish with an associated Requirement that the Rubbish [be] taken out or else that we can see was attributed to another Agent, Nick's Wife. The Requirement to take Rubbish out or else wins in the particular scenario which results in the formulation of a PROV-O Plan (an Entity which instructed a particular course of action) to take out rubbish which then started the Activity which Agent Nick undertook.
